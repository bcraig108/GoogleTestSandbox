...
PROD_OBJ += $(OBJ_DIR)/u_tcGoogleTestAssertions.o \
            ...
            $(OBJ_DIR)/u_tcPainter.o

# all rule: should be "[library] [executable] [test] [coverage]" 
all : library test coverage dox
...
# gtest directory
GTEST_DIR = /cygdrive/c/Users/Dad/Documents/EclipseProjects/gtest-1.6.0
GMOCK_DIR = /cygdrive/c/Users/Dad/Documents/EclipseProjects/gmock-1.6.0
...
# this rule defines the location of the execuatble file
...
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.cpp
	...
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -MF $(@:.o=.d) -MD -c $(@:$(OBJ_DIR)/%.o=$(SRC_DIR)/%.cpp) -o $@ | tee -a $(LOG_DIR)/build.log
...
$(TST_DIR)/%_test.xml : $(OBJ_DIR)/%.o $(OBJ_DIR)/%_test.o | $(TST_DIR)/$(TESTNAME)
	@mkdir -p $(TST_DIR) $(LOG_DIR)
	$(TST_DIR)/$(TESTNAME) --gtest_filter=$(@:$(TST_DIR)/u_%_test.xml=%)*.* --gtest_output="xml:$@" &> $(@:$(TST_DIR)/%.xml=$(LOG_DIR)/%.log)
